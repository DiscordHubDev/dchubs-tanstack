...
* **Search Params Validation (`validateSearch`)**:
  ...
  * *Implementation*: Use `Schema.decodeUnknownSync` or `Schema.decodeUnknownOption` to parse URL parameters. Provide safe fallback values upon failure to prevent UI crashes.
...
export class DatabaseError { readonly _tag = "DatabaseError"; constructor(readonly cause: unknown) {} }
export class UserNotFoundError { readonly _tag = "UserNotFoundError"; constructor(readonly id: string) {} }
...
export const getUserById = (id: string) => 
  Effect.gen(function* () {
    ...
    const user = yield* Effect.tryPromise({
      try: () => database.query.users.findFirst({
