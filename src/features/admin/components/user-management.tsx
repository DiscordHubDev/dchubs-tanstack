...
const formatDate = (d: Date | string | null) => (d ? DATE_FORMATTER.format(new Date(d)) : "—");

// ── UserCard ──────────────────────────────────────────────

const UserCard = memo(({ user, onView }: { user: UserType; onView: (user: UserType) => void }) => {
  return (
    <button
      type="button"
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-h-[90vh] max-w-[95vw] overflow-auto rounded-2xl border-zinc-800 bg-[#2b2d31] p-6 text-zinc-100 shadow-2xl backdrop-blur-xl sm:max-w-3xl">
          ...
          <div className="grid gap-6 py-4">
            <div className="flex flex-col items-start gap-4 rounded-xl bg-[#2b2d31]/50 p-4 sm:flex-row sm:items-center">
              ...
              <div className="w-full min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <h3 className="break-words font-semibold text-lg sm:text-xl">{user.name}</h3>
                  {isBanned && (
                    <Badge variant="destructive" className="bg-rose-500 text-xs">
                      已封鎖
                    </Badge>
                ...
                <div className="mt-2 space-y-1 text-zinc-400">
                  ...
                  {user.username && (
                    <p className="text-sm">
                      使用者名稱: <strong className="text-zinc-200">{user.username}</strong>
                    </p>
                  )}
                  {user.name && (
                    <p className="text-sm">
                      全域名稱: <strong className="text-zinc-200">{user.name}</strong>
                    </p>
                  )}
                </div>
              </div>
            ...
            {user.bio && (
              <div>
                <h4 className="mb-2 font-medium text-sm text-zinc-300">自我介紹 (Bio)</h4>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-lg border border-zinc-800/50 bg-[#202021] p-3">
                ...
                <p className="mt-1.5 break-all font-mono text-xs text-zinc-300">{user.id}</p>
              </div>

              <div className="rounded-lg border border-zinc-800/50 bg-[#202021] p-3">
                <h4 className="font-medium text-sm text-zinc-400">Discord ID</h4>
              ...
              <div className="rounded-lg border border-zinc-800/50 bg-[#202021] p-3">
                <h4 className="font-medium text-sm text-zinc-400">角色 (Role)</h4>
              ...
              <div className="rounded-lg border border-zinc-800/50 bg-[#202021] p-3">
                ...
                <p className="mt-1.5 text-sm text-zinc-300">{formatDate(user.createdAt)}</p>
              </div>
            </div>
            ...
            {isBanned && (
              <div className="rounded-lg border border-rose-900/50 bg-rose-950/20 p-3 sm:col-span-2">
                ...
                <p className="mt-1.5 text-rose-200 text-sm">{user.banReason ?? "未提供原因"}</p>
                {user.banExpires && (
        ...
        <DialogContent className="max-w-[95vw] rounded-2xl border-zinc-800 bg-[#2b2d31]/95 p-6 text-zinc-100 shadow-2xl backdrop-blur-xl sm:max-w-md">
          ...
          <div className="flex flex-col-reverse justify-end gap-3 sm:flex-row">
            ...
            <Button
              ...
              disabled={isLoading}
            >
              {isLoading ? "處理中..." : isCurrentlyBanned ? "確認解封" : "確認封鎖"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
...
function SocialLinks({ userSocial }: { userSocial: SocialLinksProps["userSocial"] }) {
  // 1. 安全地解析後端傳過來的資料結構
  const parseSocialData = (): SocialData => {
    ...
    try {
      ...
      if (Array.isArray(userSocial)) {
        const jsonStr = userSocial.find((item) => typeof item === "string" && item.startsWith("{"));
  ...
  return (
    <div className="mt-4">
      {/* 延續你原本 UI 的標題風格 */}
      <h4 className="mb-3 font-medium text-sm text-zinc-300 tracking-wide">社群連結 (Social)</h4>
...
export default function UserManagement() {
  ...
  const { data, isLoading, isFetchingNextPage, hasNextPage, fetchNextPage } = useInfiniteQuery(
    adminUsersInfiniteQueryOptions(debouncedSearch),
  );
  ...
  const toggleBanMutation = useMutation({
    mutationFn: (payload: { targetUserId: string; isBanned: boolean; reason?: string }) =>
        ...
        <div className="space-y-6">
          ...
          <div className="fade-in mt-4 animate-in duration-500">
            {isLoading ? (
              ...
              <EmptyState message={debouncedSearch ? "沒有符合搜尋的用戶" : "目前沒有任何用戶"} />
            ) : (
              <>
                <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-5 xl:grid-cols-3">
                  {users.map((user) => (
                    <UserCard key={user.id} user={user} onView={handleOpenDetail} />
                  ))}
                </div>
                ...
                {hasNextPage && (
                  ...
                  >
                    {isFetchingNextPage ? (
                      ...
                      <div className="text-sm text-zinc-500 opacity-50">向下滾動以載入更多</div>
                    )}
                  </div>
                )}
