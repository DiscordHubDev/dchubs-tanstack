...
const QUICK_REASONS = ["機器人功能與描述不符", "包含不適當或違規內容", "邀請連結無效"] as const;
      ...
      >
        ...
        <div className="space-y-5 py-2">
          <div className="space-y-3">
            <Label className="font-medium text-gray-300 text-sm">快速帶入原因（點擊添加）</Label>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label htmlFor="reason" className="font-medium text-gray-300 text-sm">
                拒絕原因 <span className="text-red-400">*</span>
              </Label>
              <span className={`text-xs ${charCountColor} transition-colors duration-300`}>
                {charCount} / {MAX_CHARS}
                {charCount < MIN_CHARS && (
            ...
            <div className="flex h-4 items-center">
              {charCount > 0 && charCount < MIN_CHARS && (
                <p className="fade-in slide-in-from-top-1 flex animate-in items-center gap-1.5 font-medium text-amber-400 text-xs">
                  <AlertCircle className="h-3.5 w-3.5" /> 還需要至少 {MIN_CHARS - charCount} 個字
                </p>
              )}
              {charCount > MAX_CHARS && (
                <p className="fade-in slide-in-from-top-1 flex animate-in items-center gap-1.5 font-medium text-red-400 text-xs">
                  <AlertCircle className="h-3.5 w-3.5" /> 超出 {charCount - MAX_CHARS} 個字
                </p>
              )}
            </div>
          </div>
        ...
        <DialogFooter className="mt-2 gap-3 sm:gap-2">
          ...
          >
            {isSubmitting ? (
              <>
                <span className="mr-2 inline-block animate-spin text-base">⏳</span>
                處理中...
              </>
            ) : (
              <>
