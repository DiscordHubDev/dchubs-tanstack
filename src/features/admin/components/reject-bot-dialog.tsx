...
export default memo(function RejectBotDialog({
	...
	return (
		<Dialog open={isOpen} onOpenChange={handleClose}>
			...
			<DialogContent
				className="max-h-[90vh] overflow-y-auto sm:max-w-[550px] sm:rounded-2xl shadow-2xl rounded-xl border border-[#202225] bg-[#36393F]"
				onKeyDown={handleKeyDown}
			>
				<DialogHeader className="mb-2">
					<div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
						{/* 圖示包裝：改為適合深色模式的暗紅色半透明背景與邊框 */}
						<div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-red-500/10 shadow-sm border border-red-500/20">
							<AlertCircle className="h-6 w-6 text-red-400" />
						</div>
						<div className="space-y-1">
							<DialogTitle className="text-xl sm:text-2xl font-bold tracking-tight text-gray-50">
								拒絕機器人申請
							</DialogTitle>
							{/* 描述文字改為淺灰色，機器人名稱加上更深的背景突顯 */}
							<DialogDescription className="text-gray-400 text-sm leading-relaxed">
								您即將拒絕{" "}
								<span className="font-semibold text-gray-200 bg-[#202225] px-1.5 py-0.5 rounded-md">
									{botName}
								</span>{" "}
				...
				<div className="space-y-5 py-2">
					...
					<div className="space-y-2">
						...
						<Textarea
							...
							className={`min-h-[160px] resize-none text-sm transition-all duration-300 shadow-sm bg-[#202225] border-gray-600 text-gray-200 placeholder:text-gray-500 focus-visible:shadow-md focus-visible:ring-offset-0 ${
								isOverLimit
									? "border-red-400/80 focus-visible:ring-red-500/30 focus-visible:border-red-400"
						...
						<div className="h-4 flex items-center">
							{charCount > 0 && charCount < MIN_CHARS && (
								<p className="flex items-center gap-1.5 text-amber-400 text-xs font-medium animate-in fade-in slide-in-from-top-1">
									<AlertCircle className="h-3.5 w-3.5" /> 還需要至少{" "}
									{MIN_CHARS - charCount} 個字
								</p>
							)}
							{charCount > MAX_CHARS && (
								<p className="flex items-center gap-1.5 text-red-400 text-xs font-medium animate-in fade-in slide-in-from-top-1">
					...
					<div className="rounded-xl border border-blue-500/20 bg-blue-500/10 p-3.5 shadow-sm backdrop-blur-sm">
						<p className="text-blue-200 text-xs flex items-center gap-2">
							...
							<span>
								...
								<kbd className="pointer-events-none mx-1 inline-flex h-5 items-center gap-1 rounded border border-blue-400/30 bg-blue-500/20 px-1.5 font-mono text-[10px] font-medium text-blue-200 opacity-100">
									Ctrl/Cmd + Enter
								</kbd>{" "}
								快速提交審核。
							</span>
				...
				<DialogFooter className="gap-3 sm:gap-2 mt-2">
					<Button
						...
						className="flex-1 sm:flex-initial bg-transparent border-gray-600 text-gray-300 transition-all duration-200 hover:bg-[#4F545C] hover:text-white active:scale-95 focus-visible:ring-2 focus-visible:ring-gray-500 focus-visible:ring-offset-1 focus-visible:ring-offset-[#36393F]"
					>
					...
					<Button
						...
						className="flex-1 sm:flex-initial bg-gradient-to-r from-red-600 to-red-700 text-white shadow-sm transition-all duration-200 hover:from-red-500 hover:to-red-600 hover:shadow-md active:scale-95 focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2 focus-visible:ring-offset-[#36393F] disabled:opacity-50 disabled:pointer-events-none"
					>
						{isSubmitting ? (
							<>
