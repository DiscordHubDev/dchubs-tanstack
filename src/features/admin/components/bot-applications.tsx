...
const STATUS_CONFIG = {
  pending: {
    label: "待處理",
    className: "bg-gradient-to-r from-[#FEE75C] to-[#e6d045] text-black shadow-sm font-semibold",
  },
  approved: {
    label: "已批准",
    className: "bg-gradient-to-r from-[#57F287] to-[#45d16f] text-black shadow-sm font-semibold",
  },
  rejected: {
    label: "已拒絕",
    className: "bg-gradient-to-r from-red-600 to-red-800 text-white shadow-sm font-semibold",
  },
} as const satisfies Record<BotType["status"], { label: string; className: string }>;
...
const TagList = memo(({ tags, max = 5 }: { tags: readonly string[]; max?: number }) => (
    ...
    >
      <div className="space-y-3">
        ...
        <div className="space-y-1.5 break-words text-xs sm:text-sm">
          ...
          <p className="font-medium text-gray-500 text-xs">{formatDate(app.createdAt)}</p>
        </div>
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        ...
        <DialogContent className="max-h-[90vh] max-w-3xl overflow-auto rounded-xl border border-[#202225] bg-[#36393F] text-white shadow-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-400">機器人應用詳情</DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-6 py-4">
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                ...
                <p className="font-medium text-gray-200 text-sm">{formatDate(app.createdAt)}</p>
              </div>
            </div>
...
export default function BotApplications({ applications }: { applications: readonly BotType[] }) {
  ...
  return (
    <>
      ...
      <ApplicationDetailDialog
        ...
        onReject={() => detailDialog.item && rejectDialog.open(detailDialog.item)}
      />
