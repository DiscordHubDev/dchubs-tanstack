  ...
  ({
    ...
    return (
      ...
      >
        <div className="space-y-3">
          <div className="flex items-start gap-3 sm:gap-4">
            <OptimizedImage
              ...
              width={48}
              height={48}
              className="flex-shrink-0 rounded-full object-cover shadow-sm transition-transform duration-300 group-hover:scale-105 sm:h-12 sm:w-12" // 註：可保留響應式 className 確保視覺尺寸正確
            />
            <div className="min-w-0 flex-1">
              {/* [樣式調整]: 文字強調對比度 */}
              <h3 className="line-clamp-1 break-words font-semibold text-base text-zinc-100 sm:text-lg">
                {item.name}
              </h3>
              <p className="mt-0.5 line-clamp-2 break-words text-xs text-zinc-400 sm:text-sm">
                {item.description}
              </p>
            </div>
          </div>
        ...
        {/* [樣式調整]: 分隔線顏色與間距優化 */}
        <div className="mt-2 flex items-center justify-between border-zinc-800/60 border-t pt-3">
          ...
          <Button
            ...
            className="h-8 w-8 text-rose-400 opacity-0 transition-all hover:bg-rose-500/15 hover:text-rose-300 focus:opacity-100 active:scale-90 group-hover:opacity-100"
            onClick={(e) => {
              e.stopPropagation();
              onDelete(item);
            }}
        ...
        <DialogContent className="max-h-[90vh] max-w-[95vw] overflow-auto rounded-2xl border-zinc-800 bg-[#2b2d31] p-6 text-zinc-100 shadow-2xl backdrop-blur-xl sm:max-w-3xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-xl sm:text-2xl">
              ...
              <span className="line-clamp-2 break-words font-bold tracking-tight">{item.name}</span>
            </DialogTitle>
          ...
          <div className="grid gap-6 py-4">
            <div className="flex flex-col items-start gap-4 rounded-xl bg-[#2b2d31]/50 p-4 sm:flex-row sm:items-center">
              ...
              <div className="w-full min-w-0 flex-1">
                <h3 className="break-words font-semibold text-lg sm:text-xl">{item.name}</h3>
                {isBot ? (
                  <div className="mt-2 space-y-2 text-zinc-400">
                    <p className="font-medium text-sm text-zinc-300">開發團隊</p>
                  ...
                  <div className="mt-2 space-y-2 text-zinc-400">
                    ...
                    {item.admins.length > 0 && (
                      <>
                        <p className="mt-3 font-medium text-sm text-zinc-300">伺服器管理員</p>
                        <ul className="list-inside list-disc space-y-1 text-sm marker:text-indigo-500">
                          {item.admins.map((a) => (
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-lg border border-zinc-800/50 bg-[#202021] p-3">
                ...
                <div className="mt-1.5 flex items-center gap-2 text-sm text-zinc-200">
                  ...
                  <span>{formatDate(isBot ? item.approvedAt : item.createdAt)}</span>
                </div>
              </div>
              ...
              {isBot ? (
                <>
                  <div className="rounded-lg border border-zinc-800/50 bg-[#202021] p-3">
                    <h4 className="font-medium text-sm text-zinc-400">伺服器數</h4>
                    <div className="mt-1.5 flex items-center gap-2 text-sm text-zinc-200">
                  ...
                  <div className="rounded-lg border border-zinc-800/50 bg-[#202021] p-3">
                    <h4 className="font-medium text-sm text-zinc-400">前綴指令</h4>
                    <p className="mt-1.5 break-words font-mono text-sm text-zinc-200">
                      {item.prefix ?? "—"}
                    </p>
                  </div>
                  <div className="rounded-lg border border-zinc-800/50 bg-[#202021] p-3">
                    <h4 className="font-medium text-sm text-zinc-400">官方網站</h4>
                    {item.website ? (
                      <a
                        href={item.website}
                        target="_blank"
                        ...
                        className="mt-1.5 flex items-center gap-1.5 truncate text-indigo-400 text-sm transition-colors hover:text-indigo-300 hover:underline"
                      >
                        {item.website}
                ...
                <div className="flex flex-wrap gap-2">
                  {item.tags.map((tag) => (
                    <Badge
                      ...
                      className="bg-gradient-to-r from-indigo-500 to-blue-500 text-white text-xs shadow-sm hover:from-indigo-400 hover:to-blue-400 sm:text-sm"
                    >
                      {tag}
                    </Badge>
                  ))}
...
export default function BotServerManagement({
  ...
  const { filteredBots, filteredServers, search, setSearch, remove } = useManagement({
    initialBots,
    initialServers,
  });
  ...
  const toBotItem = useCallback((b: BotType): ManagedItem => ({ ...b, kind: "bot" as const }), []);
      ...
      <CardContent className="p-4 sm:p-6">
        <div className="space-y-6">
          <div className="group relative">
            <Search className="absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2 text-zinc-500 transition-colors group-focus-within:text-indigo-400" />
            <Input
          ...
          >
            ...
            <TabsList className="grid h-auto w-full grid-cols-2 rounded-xl border border-zinc-800/40 bg-black/20 p-1 text-zinc-400">
              <TabsTrigger
                value="bots"
                className="rounded-lg font-medium text-sm transition-all duration-300 hover:text-zinc-200 data-[state=active]:bg-gradient-to-r data-[state=active]:from-indigo-600 data-[state=active]:to-blue-600 data-[state=active]:text-white data-[state=active]:shadow-sm sm:text-base"
              >
            ...
            <TabsContent value="bots" className="fade-in mt-4 animate-in duration-500">
              {filteredBots.length === 0 ? (
                <EmptyState message={search ? "沒有符合搜尋的機器人" : "沒有找到機器人"} />
              ) : (
            ...
            <TabsContent value="servers" className="fade-in mt-4 animate-in duration-500">
              {filteredServers.length === 0 ? (
                <EmptyState message={search ? "沒有符合搜尋的伺服器" : "沒有找到伺服器"} />
              ) : (
                <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-5">
