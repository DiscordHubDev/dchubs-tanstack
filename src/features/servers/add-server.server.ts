  ...
  return Effect.gen(function* () {
    ...
    if (!account?.accessToken) {
      return yield* Effect.fail(
        new Error("Discord access token is missing. Please sign in with Discord again."),
      );
    }
...
function getGuildMembershipBundleEffect(
  ...
  return Effect.gen(function* () {
    ...
    const publishedServerIdSet = new Set(publishedServerRows.map((item) => item.id));
...
export async function getGuildMembershipBundle(userId: string): Promise<GuildMembershipBundle> {
