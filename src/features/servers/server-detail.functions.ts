...
export const getServerDetailFn = createServerFn({ method: "GET" })
  ...
  .validator(effectInputValidator(ServerDetailInputSchema))
  .handler(async ({ data, context }) => {
    return getServerDetailById(data.serverId, context.user?.betterAuthId);
  });

// 2. 投票：必須登入
export const voteServerFn = createServerFn({ method: "POST" })
  .middleware([protectedMiddleware])
  .validator(effectInputValidator(ServerVoteInputSchema))
  .handler(async ({ data, context }) => {
    return voteServerById(data.serverId, context.user.betterAuthId);
  });

// 3. 評分：必須登入
export const rateServerFn = createServerFn({ method: "POST" })
  .middleware([protectedMiddleware]) // 限制登入
  .validator(effectInputValidator(ServerRateInputSchema))
  .handler(async ({ data, context }) => {
    return rateServerById(data.serverId, data.rating, context.user.betterAuthId);
  });

// 4. 檢舉：必須登入
export const reportServerFn = createServerFn({ method: "POST" })
  .middleware([protectedMiddleware])
  .validator(effectInputValidator(ServerReportInputSchema))
  .handler(async ({ data, context }) => {
