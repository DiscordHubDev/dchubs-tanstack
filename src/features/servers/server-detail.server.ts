...
function getServerDetailEffect(
  ...
  return Effect.gen(function* () {
    ...
    const [ownerRows, reviewRows, recentVoteCreatedAt, relatedRows] = yield* Effect.all(
      [
    ...
    const nextVoteAt = recentVoteCreatedAt
      ? new Date(new Date(recentVoteCreatedAt).getTime() + TWELVE_HOURS_MS).toISOString()
      : null;
...
function rateServerEffect(
  ...
  return Effect.gen(function* () {
    ...
    const targetRows = yield* dbEffect("Failed to find server for rating", () =>
      db.select({ id: server.id }).from(server).where(eq(server.id, serverId)).limit(1),
    );
...
export function voteServerById(serverId: string, userId: string): Promise<ServerVoteResult> {
