...
async function main() {
  ...
  const _queryParams = new URLSearchParams({
    filter: JSON.stringify({
      _and: [{ username: { _eq: "未知使用者" } }, { discord_id: { _nempty: true } }],
    }),
    fields: "id,discord_id",
    limit: "-1",
  });

  const fetchUsersRes = await fetch("${DIRECTUS_URL}/items/${collectionName}?${queryParams}", {
  ...
  for (const [index, user] of targetUsers.entries()) {
    ...
    try {
      // 請求 Discord API
      const discordRes = await fetch(`https://discord.com/api/v10/users/${user.discord_id}`, {
      ...
      if (!discordRes.ok) {
        if (discordRes.status === 429) {
          ...
          console.warn(`⚠️ 觸發 Discord Rate Limit！強制暫停 ${retryAfter} 秒...`);
