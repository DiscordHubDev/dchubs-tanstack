...
class DbUpdateServerPinError extends Data.TaggedError(
	"DbUpdateServerPinError",
)<{
	readonly message: string;
}> {}
		...
		try: async () => {
			const updateResult = await db
				...
				.where(
					and(
						eq(server.pin, true),
						lt(server.pinExpiry, sql`CURRENT_TIMESTAMP`),
					),
				);
...
const checkAndResetPinsProgram = Effect.gen(function* () {
	...
	if (serverResetCount > 0) {
		console.log(
			`🔄 發現並重置了 ${serverResetCount} 個過期的伺服器 (Server) Pin。`,
		);
	} else {
