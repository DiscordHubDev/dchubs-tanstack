  ...
  server: {
    handlers: {
      GET: async () => {
        ...
        const scores = bots.map((item) => (item.upvotes ?? 0) * 0.7 + (item.servers ?? 0) * 0.3);
        ...
        const fields: SitemapField[] = bots.map((item) => ({
          ...
          changefreq: item.upvotes > 500 ? "daily" : item.upvotes > 100 ? "weekly" : "monthly",
          priority: calcPriority({ upvotes: item.upvotes, servers: item.servers }, maxScore),
        }));
