...
export const auth = betterAuth({
	...
	databaseHooks: {
		user: {
			create: {
				after: async (user) => {
					await syncDomainUser(user as any);
				},
			},
			update: {
				after: async (user) => {
	...
	plugins: [
		customSession(async ({ user, session }) => {
			const discordProfile = {
				id: (user as any).discordId || user.id,
				username: (user as any).username || user.name || "",
				global_name: user.name || "",
				image_url: (user as any).avatar || user.image || "",
				avatar: (user as any).avatar || user.image || "",
				banner_url: (user as any).banner || null,
				banner_color: (user as any).bannerColor || null,
			};
