...
export function createPriorityCalculator(config: { voteWeight: number; serverWeight: number }) {
  return (
    input: { upvotes?: number | null; servers?: number | null },
    maxScore: number,
  ): number => {
    const score =
      (input.upvotes ?? 0) * config.voteWeight + (input.servers ?? 0) * config.serverWeight;
