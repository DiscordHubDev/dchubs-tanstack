...
export const user = pgTable(
  "auth_user",
  {
    ...
    image: text("image"),
    createdAt: isoDateText("created_at")
      .default(sql`(now())`)
      .notNull(),
  ...
  (table) => [index("auth_user_created_at_idx").using("btree", table.createdAt.asc().nullsLast())],
);
...
export const bot = pgTable(
  ...
  (table) => [
    ...
    index("Bot_nsfw_idx").using("btree", table.nsfw.asc().nullsLast().op("bool_ops")), // <- 為過濾 NSFW 加的索引
    foreignKey({
