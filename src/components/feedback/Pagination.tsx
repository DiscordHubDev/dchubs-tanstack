...
function Pagination({ currentPage, totalPages, onPageChange, compact = false }: PaginationProps) {
  const pageNumbers = useMemo(() => {
    if (compact) {
      ...
      if (currentPage >= totalPages - 2) {
        pages.push("ellipsis-start", totalPages - 2, totalPages - 1, totalPages);
        return pages;
      }
  ...
  const containerSpacing = compact ? "mt-2 gap-1 flex-nowrap overflow-x-hidden" : "mt-8 gap-2";
  const stableButtonClass = "hover:scale-100 active:scale-100";

  return (
    <div className={`flex w-full max-w-full items-center justify-center ${containerSpacing}`}>
      {/* 上一頁按鈕 */}
      <Button
        variant="outline"
        size={navButtonSize}
