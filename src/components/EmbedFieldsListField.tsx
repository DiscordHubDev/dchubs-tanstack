...
export default function EmbedFieldsListField({ field }: { field: AnyFieldApi }) {
  const fields = Array.isArray(field.state.value) ? (field.state.value as EmbedFieldType[]) : [];
  const [fieldKeys, setFieldKeys] = useState<string[]>(() => fields.map(() => crypto.randomUUID()));
  ...
  const updateField = (index: number, patch: Partial<EmbedFieldType>) => {
    field.handleChange(fields.map((f, i) => (i === index ? { ...f, ...patch } : f)));
  };
            ...
            >
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  id={`inline-${index}`}
                  checked={f.inline}
                  onCheckedChange={(c) => updateField(index, { inline: c === true })}
                />
                <Label
