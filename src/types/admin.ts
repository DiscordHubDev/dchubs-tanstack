...
export interface Developer extends Pick<
  BaseUser,
...
export interface ServerOwner extends Pick<BaseUser, "id" | "username" | "name"> {}
...
// 🤖 Bot 型別推導
type BotQueryConfig = DBQueryConfig<"one", true, SchemaTables, SchemaTables["bot"]> & {
  // 對應 botRelations 中的 developers 關聯，並進一步抓出 user
  with: { developers: { with: { user: true } } };
};

type DrizzleBotResult = BuildQueryResult<SchemaTables, SchemaTables["bot"], BotQueryConfig>;
...
// 🌐 DiscordServer 型別推導
type ServerQueryConfig = DBQueryConfig<"one", true, SchemaTables, SchemaTables["server"]> & {
...
type ReportQueryConfig = DBQueryConfig<"one", true, SchemaTables, SchemaTables["report"]> & {
...
export type Report = Omit<DrizzleReportResult, "attachments" | "reportedBy" | "handledBy"> & {
