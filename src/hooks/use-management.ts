...
export function useManagement({ initialBots, initialServers, onError }: UseManagementOptions) {
  const [bots, setBots] = useState<Bot[]>(() => [...initialBots]);
  const [serverList, setServerList] = useState<DiscordServer[]>(() => [...initialServers]);
