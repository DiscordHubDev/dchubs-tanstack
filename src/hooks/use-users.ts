...
import type { ToggleFavoriteParams, UserDetail, UserSummary } from "#/features/users/users.types";
...
export function useFavoriteMutation() {
  ...
  return useMutation({
    ...
    onMutate: async ({ target, id }) => {
      ...
      const previousUser = queryClient.getQueryData<UserDetail | null>(queryKeys.users.current());

      if (previousUser) {
        const fieldName = target === "server" ? "favoriteServers" : "favoriteBots";
        const currentFavorites = previousUser[fieldName] || [];
        const isFavorited = currentFavorites.some((item) => item.id === id);

        queryClient.setQueryData<UserDetail | null>(queryKeys.users.current(), (old) => {
          if (!old) return null;
